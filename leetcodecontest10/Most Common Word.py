...
class Solution:
    def mostCommonWord(self, paragraph: str, banned: List[str]) -> str:
        ...
        for word in range (len(paragraph)):
            for letter in paragraph[word]:
                if (letter in panctuation):
                    paragraph[word] = paragraph[word].replace(letter, "")
        ...
        for word in paragraph:
            if mydict.get(word) != None:
                mydict[word] += 1
                if(maximum < mydict[word]):
                    maximum = mydict.get(word)
                    result = word
            elif word not in banned and word != '':
                mydict[word] = 1
                if result == "":
                    result = word
...
main.mostCommonWord(paragraph, banned)
